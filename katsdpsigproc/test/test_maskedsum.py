...
class TestMaskedSum(object):
    def test_maskedsum(self):
        yield self.check_maskedsum, 4096, 1
    ...
    def check_maskedsum(self, R, C, context, queue):
        ...
        self.pad_dimension(fn.slots['mask'].dimensions[0], 1)
        self.pad_dimension(fn.slots['mask'].dimensions[1], 4)
        ...
        msk = np.ones((C,1)).astype(np.float32)
        ...
        expected=np.sum(data*mask,axis=0).astype(np.float32).reshape(-1)
