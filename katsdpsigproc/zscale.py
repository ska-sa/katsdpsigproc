...
def sample_image(image: np.ndarray, max_samples=100000) -> np.ndarray:
    """Take regularly-spaced samples from a 2D image.

    This provides suitable input for :func:`zscale`.
    """
    nx, ny = image.shape
    stride = int(max(1., np.sqrt(nx * ny / float(max_samples))))
